...
def sbg_linearmod(iterate):
    ...
    X_train, X_test, y_train, y_test = train_test_split(df_X, df_Y1, test_size=0.20, random_state=iterate)
    X_train = sc.fit_transform(X_train)
    X_test = sc.transform(X_test)
    ...
    dev_cpu = abs(y_pred - y_test.values)
    mean_dev = np.mean(dev_cpu)
    mse_cpu = np.sqrt(np.sum(dev_cpu ** 2) / dev_cpu.size)
    mape = np.mean(dev_cpu / y_test.values)
    max_pe = np.max(dev_cpu)
    max_ne = np.max(np.negative(dev_cpu))
    ...
    filename = r'C:\Users\epatdeb\AlphaCANDI\SBG_Rawinput_1.6\latest\Logs\AlphaCandi17_LinearReg_CPU.log'
    logging.basicConfig(filename=filename, level=logging.DEBUG)
    logging.info("tensor_bp sbg_linearmod iter:%s \n \n y_pred/y_test: \n %s \n mae:%s mse:%s mape:%s max_pe:%s max_ne:%s",
                 iterate, new_data,  mean_dev, mse_cpu, mape, max_pe, max_ne)
    logging.shutdown()
...
def sbg_linearSGD(iterate):
    ...
    filename2 = r'C:\Users\epatdeb\AlphaCANDI\SBG_Rawinput_1.6\latest\Logs\AlphaCandi17_SGD_CPU.log'
...
def sbg_lasso(iterate):
    ...
    filename3 = r'C:\Users\epatdeb\AlphaCANDI\SBG_Rawinput_1.6\latest\Logs\AlphaCandi17_Lasso_CPU.log'
...
def sbg_lassolars(iterate):
    ...
    filename4 = r'C:\Users\epatdeb\AlphaCANDI\SBG_Rawinput_1.6\latest\Logs\AlphaCandi17_LassoLars_CPU.log'
...
def sbg_bayesian(iterate):
    ...
    filename5 = r'C:\Users\epatdeb\AlphaCANDI\SBG_Rawinput_1.6\latest\Logs\AlphaCandi17_Bayesian_CPU.log'
...
def sbg_naivebayes(iterate):
    ...
    filename6 = r'C:\Users\epatdeb\AlphaCANDI\SBG_Rawinput_1.6\latest\Logs\AlphaCandi17_NaiveBayes_CPU.log'
...
def sbg_elasticnet(iterate):
    ...
    filename7 = r'C:\Users\epatdeb\AlphaCANDI\SBG_Rawinput_1.6\latest\Logs\AlphaCandi17_ElasticNet_CPU.log'
...
def sbg_ridge(iterate):
    ...
    filename8 = r'C:\Users\epatdeb\AlphaCANDI\SBG_Rawinput_1.6\latest\Logs\AlphaCandi17_Ridge_CPU.log'
...
def sbg_supportvector(iterate):
    ...
    filename9 = r'C:\Users\epatdeb\AlphaCANDI\SBG_Rawinput_1.6\latest\Logs\AlphaCandi17_SupportVector_CPU.log'
...
def sbg_polypipe(iterate):
    ...
    filename10 = r'C:\Users\epatdeb\AlphaCANDI\SBG_Rawinput_1.6\latest\Logs\AlphaCandi17_PolyPipe_CPU.log'
